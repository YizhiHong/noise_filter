...
def output_list_file(path, data_list):
    with open(path, "w") as outfile:
        ...
        outfile.write("\n".join(("%s\t%s" % (date+" " +time, val) for (date, time, val) in data_list)))
...
def visualize():
    ...
    y = list()
    for row in result:
         (date_filtered, val_filtered) = row
         x.append(date_filtered)
...
def main():
    ...
    # load data
    #
    with open('fake data.txt', "r", encoding='UTF-8') as f:    
        s = f.read()
    ...
    for i in range(0,len(records)): 
        if i%3==0:
            ...
            data_time = datetime.strptime(date + ' ' + time,'%d/%m/%Y %H:%M:%S')#for matlab
    ...
    for row in table_list:
        ...
        if i < len(datetime_range_list):
            ...
            if (not start <= data_time <= end) and val > high:
                result.append((data_time.date().strftime("%d/%m/%Y"), data_time.time().strftime("%H:%M:%S"), val))
